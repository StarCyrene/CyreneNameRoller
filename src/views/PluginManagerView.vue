    ...
    <FluentModal v-model="confirmVisible" :title="confirmTitle" max-width="560px" persistent @close="resolveConfirmation(false)">
      <div class="confirm-body" v-if="confirmMode === 'install' && confirmManifest">
        ...
        <div class="confirm-summary">
          ...
          <div><span>{{ lang === 'en' ? 'Trust' : '信任状态' }}</span><strong :class="{ danger: !confirmRisk.signed }">{{ confirmRisk.signed ? (lang === 'en' ? 'Signed and verified' : '签名已验证') : (lang === 'en' ? 'Unsigned / unverified' : '未签名 / 未验证') }}</strong></div>
        </div>
        <div class="confirm-list"><h3>{{ lang === 'en' ? 'Access summary' : '访问能力摘要' }}</h3><ul><li>{{ lang === 'en' ? 'Internet' : '网络访问' }}: {{ confirmRisk.network ? (lang === 'en' ? 'enabled' : '已启用') : (lang === 'en' ? 'disabled' : '未启用') }}</li><li>{{ lang === 'en' ? 'System execution' : '系统执行' }}: {{ confirmRisk.execute ? (lang === 'en' ? 'enabled' : '已启用') : (lang === 'en' ? 'disabled' : '未启用') }}</li><li>{{ lang === 'en' ? 'External roots' : '外部目录' }}: {{ confirmRisk.externalRoots.length }}</li><li>{{ lang === 'en' ? 'Core and host data' : '核心与界面数据' }}: {{ [...confirmRisk.coreData, ...confirmRisk.hostSurface].join(', ') || (lang === 'en' ? 'none' : '无') }}</li></ul></div>
...
import { summarizeInstallationRisk } from '../plugins/package'
...
const confirmRisk = computed(() => summarizeInstallationRisk(confirmManifest.value || {}))
