...
function buildRollerSpotlight(element, onComplete) {
  return gsap.timeline({ defaults: { ease: 'cnr-standard', overwrite: 'auto' }, onComplete })
...
function buildRollerLift(element, onComplete) {
  return gsap.timeline({ defaults: { ease: 'cnr-lift', overwrite: 'auto' }, onComplete })
...
function buildRollerGlow(element, onComplete) {
  return gsap.timeline({ defaults: { ease: 'cnr-glow', overwrite: 'auto' }, onComplete })
...
export function runFinishAnimation(element, variant = 'spotlight', style = 'roller', onComplete) {
  ...
  if (!(FINISH_BUILDERS[style] && FINISH_BUILDERS[style][variant])) console.warn('[animation] unknown finish animation', style, variant)
